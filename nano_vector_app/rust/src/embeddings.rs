...
impl TextEmbeddingModel {
    pub async fn new(local_path: Option<String>) -> Result<Self> {
        let device = Device::Cpu;

        let (config_filename, tokenizer_filename, weights_filename) = if let Some(path) = local_path {
            println!("Loading model from local path: {}", path);
            let base_path = std::path::Path::new(&path);
            (
                base_path.join("config.json").to_path_buf(),
                base_path.join("tokenizer.json").to_path_buf(),
                base_path.join("model.safetensors").to_path_buf()
            )
        } else {
            ...
            (
                repo.get("config.json").await.context("Failed to get config.json")?,
                repo.get("tokenizer.json").await.context("Failed to get tokenizer.json")?,
                repo.get("model.safetensors").await.context("Failed to get model.safetensors")?
            )
        };
